...
def test_serialize_from_array(tmpdir):
    filename = '%s' % tmpdir.join('array.xml')
    data = [{'tag': 'X-PRE-PROCESS', 'children': [], 'attrs': {} }, {'tag': 'X-PRE-PROCESS', 'children': [], 'attrs': {} }]
    fac = XMLFileFactory(data, filename, include=True)
    fac.convert()
    assert etree.fromstring(fac.getXML())